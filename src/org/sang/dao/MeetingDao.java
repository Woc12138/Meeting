...
public class MeetingDao {
    ...
    public List<Meeting> getMyMeeting(int empid) {
        ...
        try {
            con = DBUtils.getConnection();
            ps = con.prepareStatement("SELECT m.*,e.`employeename`,mr.`roomname` FROM meeting m,employee e,meetingroom mr WHERE m.`meetingid` IN(SELECT mp.`meetingid` FROM meetingparticipants mp WHERE mp.`employeeid`=?) AND m.`roomid`=mr.`roomid` AND m.`reservationistid`=e.`employeeid` and m.status=0");
            ps.setInt(1, empid);
            rs = ps.executeQuery();
            while (rs.next()) {
                int meetingid = rs.getInt("meetingid");
